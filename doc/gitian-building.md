...
*Setup instructions for a Gitian build of Acoin7 Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Acoin7
...
- [Building Acoin7 Core](#building-acoin7-core)
...
Clone the git repositories for acoin7 and Gitian.
...
in the acoin7 repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Acoin7 Core
----------------

To build Acoin7 Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the acoin7 repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/acoin7/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/acoin7-project/acoin7
...
For example:
```bash
URL=https://github.com/thrasher-/acoin7.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit acoin7=${COMMIT} --url acoin7=${URL} ../acoin7/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit acoin7=${COMMIT} --url acoin7=${URL} ../acoin7/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit acoin7=${COMMIT} --url acoin7=${URL} ../acoin7/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the acoin7 git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../acoin7/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/acoin7
SIGPATH=/some/root/path/acoin7-detached-sigs

./bin/gbuild --url acoin7=${BTCPATH},signature=${SIGPATH} ../acoin7/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/acoin7-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/acoin7-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/acoin7-osx-build.assert
```
...
[acoin7-project/gitian.sigs.ltc](https://github.com/acoin7-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
