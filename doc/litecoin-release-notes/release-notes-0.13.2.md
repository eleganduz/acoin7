Acoin7 Core version 0.13.2 is now available from:

  <https://download.acoin7.org/acoin7-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/acoin7-project/acoin7/issues>
...
updates will be released anymore. Without security updates, using a acoin7
...
ECDSA signatures inside Acoin7 transactions now use validation using
...
Previous versions of Acoin7 Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Acoin7 Core was to raise the default minimum
relay fee.

Acoin7 Core 0.13.2 will have a strict maximum size on the mempool. The
...
Acoin7 Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running acoin7d without having to do any manual configuration.
...
Acoin7 Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Acoin7 Core will relay transactions with
...
entirely in Acoin7 Core 0.13, and it is at this time undecided whether the
...
Acoin7 Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Acoin7 Core automatically creates a hidden service to listen on, without
manual configuration. Acoin7 Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Acoin7 Core is listening, and
...
Acoin7d can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Acoin7
Core to use floating fees. By default, Acoin7 Core will use floating
fees.
...
At all times, Acoin7 Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Acoin7 Core will never create transactions paying less than
...
`acoin7.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /acoin7rpc>
        ...
        # optional bypass acoin7d rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< acoin7rpc:<password>
    </Location>
...
Restarting acoin7d will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in acoin7.conf
...
For example:
    ...
    $ src/acoin7-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Acoin7 Core code base has
...
- `acoin7-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `acoin7-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Acoin7 scripting system that in combination with
...
Acoin7 transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Acoin7 Core.
...
The two phases are distinct in the Acoin7-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Acoin7 wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Acoin7.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Acoin7 block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of acoin7s they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of acoin7s being spent is exactly
...
- **Increased security for multisig:** Acoin7 addresses (both P2PKH addresses
  ...
  believed to be provided by Acoin7's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Acoin7's current ledger (called the UTXO set),
  ...
  note that Acoin7 Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Acoin7 users to individually opt-in to almost any change in the Acoin7
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Acoin7 Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Acoin7
client to Acoin7 Core 0.13.2 to signal for segwit support. When a
...
Since Acoin7 Core 0.10.0, nodes have defaulted to only relaying and
...
- `acoin7-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `acoin7-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
