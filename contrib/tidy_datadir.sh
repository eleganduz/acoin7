...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Acoin7 database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Acoin7 datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Acoin7 datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Acoin7 0.7 datadir."
    ;;
  3)
    echo "Detected Acoin7 pre-0.8 datadir."
    ;;
  4)
    echo "Detected Acoin7 0.8 datadir."
    ;;
esac
